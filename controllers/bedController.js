...
const { Bed, IPDAdmission, Room, Ward, Patient, Doctor,SubDoctor,Appointment, IPDAccount,IPDAccountCharge,IPDAccountPayment,sequelize } = require("../models");
const { decrypt } = require("../utils/cryptography");
const generatePaymentId = require("../utils/generatePaymnetId");
const generateAccountNumber = require("../utils/generateAccountNumber");

const getDecryptedName = (person) =>
  person?.name ? decrypt(person.name) : null;
...
exports.updateBed = async (req, res) => {
  ...
  try {
    ...
    const bed = await Bed.findOne({ where: { id: req.params.id, doctorId:hospitalId }, transaction });
    ...
    if (roomId || wardId || roomNumber) {
      nextRoom = await findRoomForBed({ doctorId:hospitalId, roomId, wardId, roomNumber, transaction });
      if (!nextRoom) {
        await transaction.rollback();
        return res
          .status(404)
    ...
    if (nextStatus !== "occupied") {
      const activeAdmission = await IPDAdmission.findOne({
        where: { hospitalId, bedId: bed.id, status: "admitted" },
        transaction,
      });
    ...
    if (nextStatus === "occupied") {
      if (patientRecord?.admission) {
        ...
        return res
          .status(404)
          .json({ success: false, message: "Patient is already admitted. Please use the bed transfer functionality to change the bed." });
        ...
        const isDoctor = !!lastAppointment.doctorId;

        const name = isDoctor
          ? getDecryptedName(lastAppointment.doctor)
          : getDecryptedName(lastAppointment.subDoctor);

        const department = isDoctor
          ? lastAppointment.doctor?.department
          : lastAppointment.subDoctor?.department;

        const patientUniqueId = lastAppointment?.patient?.patientId;
        ...
        const admission = await IPDAdmission.create(
          {
            ...
            doctorId: isDoctor ? lastAppointment.doctorId : null,
            subDoctorId: isDoctor ? null : lastAppointment.subDoctorId,
        ...
        const accountNumber = await generateAccountNumber(
          IPDAccount,hospitalId,transaction
        );
