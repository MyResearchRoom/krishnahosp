...
const formatRoom = (room) => {
  ...
  const totalBeds =plainRoom.beds.length;
...
exports.createRoom = async (req, res) => {
  try {
    ...
    const existingRoomCount = await Room.count({ where: { wardId: ward.id } });

    if (existingRoomCount >= ward.capacity) {
      return res.status(400).json({
        success: false,
        message: "Ward capacity is already full to add room under ward.",
      });
    }
