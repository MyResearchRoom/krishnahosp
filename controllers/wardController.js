...
const summarizeBeds = (capacity, beds = []) => {
  ...
  const availableBeds = Math.max(beds.length - occupiedBeds - maintenanceBeds, 0);
  
  const totalBeds=beds.length;

  return { availableBeds, occupiedBeds ,totalBeds};
};

const formatWard = (ward) => {
  const plainWard = ward.toJSON();
  const { availableBeds, occupiedBeds, totalBeds} = summarizeBeds(
    plainWard.capacity,
    plainWard.beds
  );

  const availableRooms=plainWard.rooms.length;

  delete plainWard.beds;
  delete plainWard.rooms;
