...
exports.getSections = async (req, res) => {
  try {
    ...
    const admission = await IPDAdmission.findOne({ where: { id: admissionId, hospitalId:doctorId } });
    if (!admission) {
...
exports.createSection = async (req, res) => {
  ...
  try {
    ...
    const admission = await IPDAdmission.findOne({ where: { id: admissionId, hospitalId:doctorId }, transaction });
    if (!admission) {
      await transaction.rollback();
      return res.status(404).json({ success: false, message: "Admission not found." });
    }
